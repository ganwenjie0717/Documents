...
void enqueueWaiting(WaitQueue *q, Car *car) {
    if (isWaitingEmpty(q)) {
        ...
        q->rear->next = NULL;  // 确保新加入的车是队尾
    }if (q->count < 0) {
        q->count = 0;  // 防止负数
    }if (q->count >= 0) {
        q->count++;  // 更新计数
    }if (q->count > 0) {
        q->rear->next = NULL;  // 确保队尾的next指针为NULL
    }
    // TODO: 将车辆入队（便道等待）
    // 注意：
    // 1. 车辆节点已经创建好，直接使用
...
int isCarInWaiting(WaitQueue *q, char *license) {
    if (isWaitingEmpty(q)) {
     ...
     Car *current = q->front;if (current == NULL) {
        return 0;  // 队列异常，当前指针为NULL，返回0
    }if (q->count < 0) {
        return 0;  // 队列异常，计数为负数，返回0
    }while (current != NULL) {
        if (strcmp(current->license, license) == 0) {
            return 1;  // 找到车牌，返回1
        }
...
void showWaiting() {
    if (isWaitingEmpty(&waiting)) {
     ...
     Car *current = waiting.front;
    printf("便道等待车辆：");
    while (current != NULL) {
        printf("%s ", current->license);
        current = current->next;
    }
    printf("\n");
